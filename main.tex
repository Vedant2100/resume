...
\section{Experience}
\noindent\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r}
  \textbf{Research Intern} & \textbf{Jun 2026 - Present} \\
  \textit{CompFly AI} & \textit{San Francisco, CA} \\
\end{tabular*}\vspace{-7pt}
\resumeItemListStart
\resumeItem{Engineered \textbf{agentic observability} for the agent-monitoring platform: a nested \textbf{OpenTelemetry} span tree tracing each agent's full causal trajectory -- agent turns, tool calls, and sub-agent handoffs with token-level cost -- normalized across six frameworks (OpenAI, Anthropic, AutoGen, Bedrock, LlamaIndex, LangChain).}
\vspace{-3pt}
\resumeItem{Extending platform governance beyond RAG retrieval and tool-call policy enforcement to a new \textbf{memory-governance} layer -- controls and audit over agent memory reads/writes to mitigate context poisoning and cross-session data leakage.}
\resumeItemListEnd
\vspace{-2pt}
...
\vspace{-3pt}
\resumeItem{Performed comprehensive time-series diagnostics using Autocorrelation plots \& extended autocorrelation matrices, stationarity tests (mean, variance), and residual analysis; engineered lagged and augmented data using exogenous features (e.g., mortality rates, inventory, veteran population) to enhance model predictive power.}
\vspace{-3pt}
...
\begin{itemize}[leftmargin=0.15in, label={}]
    \small{\item{\textbf{Languages:} C++, Python, R, JavaScript, Bash, Prolog, AWK, MIPS, Neo4J CQL, SQL, Sed, Lex, Yacc, \LaTeX}}
    \vspace{-5pt}\small{\item{\textbf{Technologies:} PyTorch, TensorFlow, Pandas, NumPy, SciPy, React, Django, PostgreSQL, MongoDB, Power BI}}
    \vspace{-5pt}\small{\item{\textbf{AI \& Frameworks:} LangChain, LlamaIndex, AutoGen, OpenTelemetry, Prophet, SARIMA, LSTM, Qwen}}
    \vspace{-5pt}\small{\item{\textbf{Software:} Git, Docker, Cursor, Azure DevOps, Jira, Confluence, DataGrip, RStudio, Excel}}
    \vspace{-5pt}\small{\item{\textbf{Coursework:} Foundation Models, Adv. Computer Vision, Deep Learning, Big-Data Mgmt., Adv. Algorithms, Adv. OS}}
\end{itemize}
\vspace{-15pt}
%-----------ACCOMPLISHMENTS-----------
