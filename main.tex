...
\vspace{-5pt}
\resumeItemListStart
\resumeItem{Benchmark frontier models (GPT, Claude, Gemini, Llama, Mistral) against adversarial attack vectors mapped to the \textbf{OWASP LLM Top 10} and \textbf{MITRE ATLAS}, designing multi-step, multi-agent red-team suites and curating the proprietary datasets that train CompFly's detection and defense models.}
\vspace{-3pt}
...
\resumeItem{Developed an \textbf{end-to-end forecasting pipeline} encompassing data preprocessing, feature engineering, model selection with custom metrics, and Docker-based containerization for reproducible deployment across supply chain nodes.}
    \vspace{-3pt}
    \resumeItem{Built \textbf{AI agents} for RAG-based feature engineering (+5\% model performance), natural-language cross-database querying via SQL/Python code generation, and automated KPI business dashboards.}
    \vspace{-3pt}
