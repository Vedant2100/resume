...
\end{tabular*}\vspace{-7pt}
\resumeItemListStart
\resumeItem{Engineered \textbf{agentic observability} for the agent-monitoring platform: a nested \textbf{OpenTelemetry} span tree tracing each agent's full causal trajectory, including agent turns, tool calls, and sub-agent handoffs with token-level cost, normalized across leading AI and agent frameworks.}
\vspace{-3pt}
\resumeItem{Extending platform governance beyond RAG retrieval and tool-call policy enforcement to a new \textbf{memory-governance} layer with controls and audit over agent memory reads/writes to mitigate context poisoning and cross-session data leakage.}
\resumeItemListEnd
\vspace{2pt}
\noindent\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r}
  \textbf{Data Scientist} & \textbf{Aug 2023 - Aug 2025} \\
  \textit{Finarb AI} & \textit{Kolkata, India} \\
\end{tabular*}\vspace{-7pt}
